      ...
      <section className="pt-32 pb-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-5xl md:text-7xl font-bold tracking-tight mb-6" style={{ color: '#FFFFFF', lineHeight: 1.1 }}>
            <span style={{ color: '#9D4EDD' }}>{t('home.heroLine1', 'Creator Commerce')}</span>
            <br />
            {t('home.heroLine2', 'Platform')}
          </h1>
          <p className="text-lg mb-10 max-w-2xl mx-auto" style={{ color: 'rgba(255,255,255,0.7)' }}>
            {t('home.heroDesc', 'Connect with top creators, launch products, and grow your brand in the 3C accessories space.')}
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/register?role=creator">
              <Button size="lg" className="w-full sm:w-auto" style={{ backgroundColor: '#FF006E', color: '#fff' }}>
                {t('home.startTrial', 'Start Free Trial')}
              </Button>
            </Link>
            <Link href="/choose-role">
              <Button variant="outline" size="lg" className="w-full sm:w-auto border-white/20 text-white hover:bg-white/10" style={{ backgroundColor: 'transparent' }}>
                {t('home.joinAsCreator', 'Join as Creator')}
              </Button>
            </Link>
          </div>
        </div>
      ...
      <section className="py-20 px-6" style={{ backgroundColor: '#1A1A2E' }}>
        <div className="max-w-5xl mx-auto">
          <h2 className="text-3xl font-bold text-center mb-12" style={{ color: '#FFFFFF' }}>
            {t('home.choosePath', 'Choose Your Path')}
          </h2>
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="rounded-lg p-6 border transition-all hover:scale-105 cursor-pointer" style={{ backgroundColor: '#0F111A', borderColor: '#9D4EDD' }}>
              ...
              <h3 className="text-xl font-semibold mb-2" style={{ color: '#FFFFFF' }}>{t('home.creator', 'Creator')}</h3>
              <p className="text-sm mb-4" style={{ color: 'rgba(255,255,255,0.6)' }}>
                {t('home.creatorDesc', 'Get free samples, create content, and earn commissions')}
              </p>
            ...
            <div className="rounded-lg p-6 border transition-all hover:scale-105 cursor-pointer" style={{ backgroundColor: '#0F111A', borderColor: '#00D9FF' }}>
              ...
              <h3 className="text-xl font-semibold mb-2" style={{ color: '#FFFFFF' }}>{t('home.merchant', 'Merchant')}</h3>
              <p className="text-sm mb-4" style={{ color: 'rgba(255,255,255,0.6)' }}>
                {t('home.merchantDesc', 'List products, recruit creators, and grow your business')}
              </p>
              <Link href="/register?role=merchant" className="block">
                <Button variant="outline" className="w-full border-white/20 text-white hover:bg-white/10" style={{ backgroundColor: 'transparent' }}>
                  {t('home.merchantJoin', 'Merchant Join')}
                </Button>
              </Link>
            </div>
            ...
            <div className="rounded-lg p-6 border transition-all hover:scale-105 cursor-pointer" style={{ backgroundColor: '#0F111A', borderColor: '#00F5D4' }}>
              ...
              <h3 className="text-xl font-semibold mb-2" style={{ color: '#FFFFFF' }}>{t('home.buyer', 'Buyer')}</h3>
              <p className="text-sm mb-4" style={{ color: 'rgba(255,255,255,0.6)' }}>
                {t('home.buyerDesc', 'Use links and coupons to resell and earn commissions')}
              </p>
              <Link href="/register?role=buyer" className="block">
                <Button variant="outline" className="w-full border-white/20 text-white hover:bg-white/10" style={{ backgroundColor: 'transparent' }}>
                  {t('home.joinAsBuyer', 'Join as Buyer')}
                </Button>
              </Link>
            </div>
          </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold mb-2" style={{ color: '#FFFFFF' }}>{t('home.getProducts', 'Get Products')}</h3>
              <p className="text-sm" style={{ color: 'rgba(255,255,255,0.6)' }}>
                {t('home.getProductsDesc', 'Apply for free samples from brands looking for creators')}
              </p>
            </div>
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold mb-2" style={{ color: '#FFFFFF' }}>{t('home.createContent', 'Create Content')}</h3>
              <p className="text-sm" style={{ color: 'rgba(255,255,255,0.6)' }}>
                {t('home.createContentDesc', 'Share authentic reviews on TikTok, Instagram, or YouTube')}
              </p>
            </div>
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold mb-2" style={{ color: '#FFFFFF' }}>{t('home.earnCommission', 'Earn Commission')}</h3>
              <p className="text-sm" style={{ color: 'rgba(255,255,255,0.6)' }}>
                {t('home.earnCommissionDesc', 'Get paid for every sale made through your unique link')}
              </p>
            </div>
          </div>
        </div>
      ...
      <footer className="py-8 px-6 border-t" style={{ borderColor: 'rgba(255,255,255,0.1)' }}>
        <div className="max-w-5xl mx-auto text-center">
          <p className="text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>
            {t('home.footer', '© 2024 Kolink. All rights reserved.')}
          </p>
        </div>
      </footer>
    </div>
